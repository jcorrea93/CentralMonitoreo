...
class TokensControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post tokens_url, params: { token: { name: @token.name, number_requests: @token.number_requests, token: @token.token } }
    end
  ...
  test "should update token" do
    patch token_url(@token), params: { token: { name: @token.name, number_requests: @token.number_requests, token: @token.token } }
